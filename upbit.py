...
class CurrentPrice(Resource):
    ...
    def get(self, code, ticker):
        ...
        try:
            ...
            price = pyupbit.get_orderbook(code+pairSeporator+ticker)["orderbook_units"][0]["ask_price"]
        except:
            msg = errorText
        return {
            'ticker': ticker,
...
class StartPrice(Resource):
    ...
    def get(self, code, ticker):
        ...
        try:
            """Upbit ticker에 해당하는 현재 가격을 가져옵니다.."""
            print(pyupbit.get_orderbook(code+pairSeporator+ticker)["orderbook_units"])
            price = pyupbit.get_orderbook(code+pairSeporator+ticker)["orderbook_units"][0]["ask_price"]
        except:
            msg = errorText
        return {
...
# @Upbit.route('/moving-average/<string:code>/<string:ticker>/<string:interval>/<int:count>')
...
#         try:
#             """5일 이동 평균선 조회"""
#             df = pyupbit.get_ohlcv(code+pairSeporator+ticker, interval=interval, count=count)
...
class TargetPrice(Resource):
    ...
    def get(self, code, ticker, k):
        ...
        try:
            ...
            df = pyupbit.get_ohlcv(code+pairSeporator+ticker, interval='day', count=2) # 당일과 전일 데이터 취득
...
@Upbit.route('/predict-price/<string:code>/<string:ticker>/<string:interval>/<int:count>')
@Upbit.doc(params={'code': 'KRW', 'ticker': 'BTC', 'interval': 'minute30', 'count': 144})
class PredictPrice(Resource):
    ...
    def get(self, code, ticker, count, interval):
        ...
        closeDf = forecast[forecast['ds'] == forecast.iloc[-1]['ds'].replace(hour=9)] # 9시 마감이라 9시
        if len(closeDf) == 0:
            closeDf = forecast[forecast['ds'] == data.iloc[-1]['ds'].replace(hour=9)] # 9시 마감이라 9시
